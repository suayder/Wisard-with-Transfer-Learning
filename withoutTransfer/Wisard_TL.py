...
p_dataset = np.loadtxt("dataSet/datasetPositive.txt", delimiter=" ")
n_dataset = np.loadtxt("dataSet/datasetNegative.txt", delimiter=" ")
...
for inp in range(9,15):
    n_inputs_ram = inp
    size_Wisard = math.floor(89401/n_inputs_ram)
    results = []
    negative_results = []
    for it in range(21):
        print("LOOP: ", it)
    ...
    result = open('../results/WisardWithOutTL.dat', 'a')
